...
export const metadata: Metadata = {
  ...
  description: "Select the best messaging platform to connect to your ReplyBase workspace.",
};
...
const channels = [
  {
    ...
    icon: Globe,
    color: "bg-indigo-600",
    description: "Best for websites. A lightweight chat widget that lives on your site.",
  ...
  },
  {
    ...
    icon: MessageSquare,
    color: "bg-blue-600",
    description: "Best for social presence. Connect your official Facebook Page.",
  ...
  return (
    ...
    >
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className={`flex items-center justify-center h-10 w-10 rounded-lg ${channel.color} text-white shadow-lg`}>
                  <Icon size={20} />
                </div>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${channel.plan === 'Free' ? 'bg-emerald-600/20 text-emerald-400' : 'bg-amber-600/20 text-amber-400'}`}>
                  {channel.plan}
                </span>
              </div>
              ...
              <ul className="space-y-2 mb-6">
                {channel.features.map((feat) => (
                  <li key={feat} className="text-xs text-slate-500 flex items-center gap-2">
      ...
      <div className="mt-16 pt-8 border-t border-slate-800 flex justify-between">
        <Link href="/docs/get-started/account-setup" className="flex items-center gap-2 text-slate-500 hover:text-white transition-colors text-sm">
